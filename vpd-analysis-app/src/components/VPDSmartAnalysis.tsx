...
const VPDSmartAnalysis: React.FC<VPDTemporalAnalysisProps> = ({
  data
}) => {
  ...
  const renderIslandRecommendations = (island: string, week: number) => {
    ...
    console.log(`Rendering recommendations for ${island}, stats:`, stats);
    
    if (!stats) {
      return (
        <div className="island-recommendations-card">
          <div className="recommendation-header">
            <h4>{getIslandIcon(island)} {island} - Sin Datos</h4>
            <span className="status-badge critical">❌ Sin Info</span>
          </div>
          <p>No hay estadísticas disponibles para esta isla</p>
        </div>
      );
    }
    
    // Aplicar los mismos filtros que se usan en processedData
    let filteredData = data.data;
    ...
    if (localPeriod === 'day') {
      filteredData = data.data.filter(record => {
        const hour = record.hour;
        return hour >= 6 && hour < 17; // Día: 6:05am - 16:55pm
      });
    } else if (localPeriod === 'night') {
      filteredData = data.data.filter(record => {
        const hour = record.hour;
        return hour >= 17 && hour < 23; // Noche planta: 17:00 - 23:00
      });
    }
      ...
      filteredData = filteredData.filter(record => {
        ...
        switch (localTimeBlock) {
          case 'noche_planta':
            return hour >= 17 && hour <= 23; // Noche planta: 17:00 - 23:00
          case 'dia_planta':
            return hour >= 0 && hour < 17; // Día planta: 00:00 - 16:55
          default:
            return true;
        }
      });
    ...
    // Obtener registros de la isla con filtros aplicados
    const islandRecords = filteredData
      .filter(record => {
        const islandData = record.islands[island as keyof typeof record.islands];
        return islandData && islandData.vpd !== undefined;
      })
      .slice(-10); // Últimos 10 registros del período filtrado
    
    console.log(`Records for ${island}:`, islandRecords.length, islandRecords.slice(-1));
    ...
    if (islandRecords.length === 0) {
      return (
        <div className="island-recommendations-card">
          ...
          <p>No hay datos disponibles para esta isla</p>
        </div>
      );
    }
    
    const lastRecord = islandRecords[islandRecords.length - 1];
    console.log(`Last record for ${island}:`, lastRecord);
    
    if (!lastRecord) {
      return (
        <div className="island-recommendations-card">
          <div className="recommendation-header">
            <h4>{getIslandIcon(island)} {island} - Sin Datos Recientes</h4>
            <span className="status-badge critical">❌ Sin Info</span>
          </div>
          <p>No hay registros recientes para esta isla</p>
        </div>
      );
    }
    ...
    const adjustments = calculateVPDAdjustments(currentTemp, currentRH, currentVPD, targetVPD);
    const vpdStatus = currentVPD >= weekConfig.optimalMin && currentVPD <= weekConfig.optimalMax ? 'optimal' :
                     currentVPD >= weekConfig.acceptableMin && currentVPD <= weekConfig.acceptableMax ? 'acceptable' : 'critical';
    
    // Determinar información del período para mostrar en el header
    const getPeriodInfo = () => {
      if (localTimeBlock === 'noche_planta') {
        return 'Noche Planta (17:00-23:00)';
      } else if (localTimeBlock === 'dia_planta') {
        return 'Día Planta (00:00-16:55)';
      } else if (localPeriod === 'day') {
        return 'Período Diurno (6:05-16:55)';
      } else if (localPeriod === 'night') {
        return 'Período Nocturno (17:00-23:00)';
      }
    ...
    return (
      <div className="island-recommendations-card">
        <div className="recommendation-header">
          <h4>{getIslandIcon(island)} {island} - Estado Actual</h4>
          <div className="header-info">
            <span className={`status-badge ${vpdStatus}`}>
              {vpdStatus === 'optimal' ? '✅ Óptimo' : vpdStatus === 'acceptable' ? '⚠️ Aceptable' : '❌ Crítico'}
            </span>
        ...
        <div className="current-metrics">
          <div className="metric">
            ...
            <span className="metric-value">{currentTemp?.toFixed(1)}°C</span>
          </div>
          <div className="metric">
            <span className="metric-label">Humedad</span>
            <span className="metric-value">{currentRH?.toFixed(1)}%</span>
          </div>
          <div className="metric">
            <span className="metric-label">VPD Actual</span>
            <span className="metric-value vpd">{currentVPD?.toFixed(2)} kPa</span>
          </div>
          <div className="metric">
            <span className="metric-label">Desviación</span>
            <span className="metric-value deviation">
              {currentVPD > weekConfig.optimalMax ? '+' : ''}{(currentVPD - targetVPD).toFixed(2)} kPa
            </span>
          </div>
        </div>
        ...
        {vpdStatus !== 'optimal' && (
          <>
            <div className="recommendations-section">
              <h5>🎯 Recomendaciones de Ajuste</h5>
              
              <div className="adjustment-options">
                <div className={`adjustment-option ${adjustments.recommendedAction === 'temperature' ? 'recommended' : ''}`}>
                  <div className="option-header">
                    <span>Opción A: Ajustar Temperatura</span>
                    {adjustments.recommendedAction === 'temperature' && <span className="recommended-badge">⭐ Recomendado</span>}
                  </div>
                  <div className="option-details">
                    <p>Cambiar a: <strong>{adjustments.tempForTargetVPD.toFixed(1)}°C</strong> ({adjustments.tempAdjustment > 0 ? '+' : ''}{adjustments.tempAdjustment.toFixed(1)}°C)</p>
                    <p className="energy-impact">⚡ {adjustments.tempEnergyImpact.toFixed(0)}W</p>
                  </div>
                </div>
                
                <div className={`adjustment-option ${adjustments.recommendedAction === 'humidity' ? 'recommended' : ''}`}>
                  <div className="option-header">
                    <span>Opción B: Ajustar Humedad</span>
                    {adjustments.recommendedAction === 'humidity' && <span className="recommended-badge">⭐ Recomendado</span>}
                  </div>
                  <div className="option-details">
                    <p>Cambiar a: <strong>{adjustments.rhForTargetVPD.toFixed(1)}%</strong> ({adjustments.rhAdjustment > 0 ? '+' : ''}{adjustments.rhAdjustment.toFixed(1)}%)</p>
                    <p className="energy-impact">⚡ {adjustments.rhEnergyImpact.toFixed(0)}W</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="trend-analysis">
              <h5>📈 Análisis de Tendencia {localTimeBlock ? `(${localTimeBlock === 'noche_planta' ? 'Noche Planta' : 'Día Planta'})` : '(Período completo)'}</h5>
              {(() => {
                // Usar los registros ya filtrados de la isla
                if (islandRecords.length < 2) {
                  const periodInfo = localTimeBlock ? 
                    (localTimeBlock === 'noche_planta' ? 'período nocturno (17:00-23:00)' : 'período diurno (00:00-16:55)') :
                    'período seleccionado';
                  return <p className="trend-info">Datos insuficientes para análisis de tendencia en {periodInfo}</p>;
                }
                
                const firstRecord = islandRecords[0];
                const lastRecord = islandRecords[islandRecords.length - 1];
                const firstIslandData = firstRecord.islands[island as keyof typeof firstRecord.islands];
                const lastIslandData = lastRecord.islands[island as keyof typeof lastRecord.islands];
                
                if (!firstIslandData || !lastIslandData) {
                  return <p className="trend-info">Datos insuficientes para análisis de tendencia</p>;
                }
                
                const firstVPD = firstIslandData.vpd;
                const lastVPD = lastIslandData.vpd;
                const trend = lastVPD - firstVPD;
                const trendDirection = trend > 0.05 ? 'subiendo' : trend < -0.05 ? 'bajando' : 'estable';
                const trendIcon = trend > 0.05 ? '📈' : trend < -0.05 ? '📉' : '➡️';
                
                return (
                  <div className="trend-info">
                    <span className="trend-indicator">
                      {trendIcon} VPD {trendDirection} ({trend > 0 ? '+' : ''}{trend.toFixed(3)} kPa)
                    </span>
                    {trendDirection !== 'estable' && (
                      <span className="trend-alert">
                        {trendDirection === 'subiendo' ? 
                          '⚠️ Tendencia al alza - Considerar ajuste preventivo' : 
                          '✅ Tendencia a la baja - Mejorando automáticamente'
                        }
                      </span>
                    )}
                  </div>
            ...
            <div className="action-buttons">
              <button className="action-button primary">
                Aplicar {adjustments.recommendedAction === 'temperature' ? 'Cambio Temperatura' : 'Cambio Humedad'}
              </button>
            ...
            <div key={week} className="week-section">
              ...
              {islandsInWeek.length > 0 && (
                <div className="recommendations-panel">
                  <h3 className="panel-title">🎯 Recomendaciones Inteligentes</h3>
                  <div className="recommendations-grid">
                    {islandsInWeek.map(island => {
                      console.log(`Processing island ${island} for week ${week}`);
                      return (
                        <div key={`${island}-${week}`}>
                          {renderIslandRecommendations(island, week)}
                        </div>
                      );
                    })}
                  </div>
...
export default VPDSmartAnalysis;