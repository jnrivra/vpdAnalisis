...
class VPDDataService {
  ...
  private currentDataSource: DataSource = 'json';
  private excelData: VPDData | null = null;
  ...
  async getData(): Promise<VPDData> {
    // Si estamos usando datos de Excel, devolverlos
    if (this.currentDataSource === 'excel' && this.excelData) {
    ...
    // Si no, hacemos fetch de los datos JSON
    try {
  ...
  /**
   * Establece datos desde Excel
   */
  setExcelData(data: VPDData): void {
    this.excelData = this.processData(data);
    this.currentDataSource = 'excel';
    // No limpiar caché aquí para evitar re-fetch innecesario
  }
  ...
  setDataSource(source: DataSource): void {
    ...
    // No limpiar caché automáticamente
  }
...
export const setExcelData = (data: VPDData) => vpdDataService.setExcelData(data);
export const setDataSource = (source: DataSource) => vpdDataService.setDataSource(source);
export const getCurrentDataSource = () => vpdDataService.getCurrentDataSource();
