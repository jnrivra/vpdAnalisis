...
export interface VPDMetadata {
  date: string;
  endDate?: string; // Para rangos de fecha desde Excel
  sector?: string; // Para identificar la parcela/sector
  totalRecords: number;
  timeInterval: string;
  islands: string[];
  processedAt?: string;
...
export type TimeBlock = 'noche_planta' | 'dia_planta' | 'madrugada' | 'manana' | 'tarde';

// Tipo para sub-bloques del día
export type DaySubBlock = 'madrugada' | 'manana' | 'tarde';
